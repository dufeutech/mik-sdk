...
<Aside type="caution">
  `require()` will panic if the variable is not set. Use it only for variables
  that are absolutely required for your handler to function.
</Aside>
...
<Aside type="note">
  Environment variables are read at runtime. In WASI environments, the runtime
  must provide access to environment variables for these functions to work.
</Aside>
