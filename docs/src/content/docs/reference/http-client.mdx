...
The `fetch!` macro is included in `mik_sdk::prelude::*` when the `http-client` feature is enabled.
...
fn fetch_data(_req: &Request) -> Response {
    ...
    if response.is_success() {
        ok!({ "data": json::try_parse(&response.body()) })
    } else {
        error! {
            status: 502,
...
// Body as bytes
let body = response.body();  // Vec<u8>

// Parse JSON
if let Some(json) = json::try_parse(&body) {
...
<Aside type="caution">
Only use `.deny_private_ips()` for user-provided URLs. For known internal service URLs, omit this to allow internal communication.
</Aside>
...
fn call_downstream(req: &Request) -> Response {
    let response = fetch!(GET "https://api.example.com/data")
        .with_trace_id(req.trace_id())  // Forwards x-trace-id header
        .send()?;
...
This adds the `x-trace-id` header to outbound requests if present in the incoming request.
...
| `.with_trace_id(opt)` | Add trace ID header |
