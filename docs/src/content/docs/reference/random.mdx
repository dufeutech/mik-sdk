...
<Aside type="tip">
  All random functions in this module use cryptographically secure randomness.
  They are suitable for: - Session tokens - API keys - Password reset tokens -
  Encryption keys - UUIDs - Any security-sensitive random values
</Aside>

<Aside type="caution">
  The functions will panic if the underlying random number generator fails. This
  is extremely rare and indicates a critical system issue.
</Aside>
