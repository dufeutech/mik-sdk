...
- `parse_filter(&str)` - Parse from string (use with `req.text()`)
- `parse_filter_bytes(&[u8])` - Parse from bytes (use with `req.body()`)
...
```rust
use mik_sql::{parse_filter_bytes, sql_read};

fn search(query: SearchQuery, req: &Request) -> Response {
    // Extract body as bytes
    let body = ensure!(req.body(), 400, "Filter body required");

    // Parse filter from bytes
    let filter = ensure!(parse_filter_bytes(body), 400, "Invalid filter");
